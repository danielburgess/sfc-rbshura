...
.DESCRIPTION
  Windows equivalent of scripts/build.sh. Builds the ROM named by project.toml's
  [rom] name into out\ (+ .ips and .xdelta) from the pristine Japanese ROM via
  retrotool, then runs the
...
Write-Host "==> OK: ROM built into out\ and audited." -ForegroundColor Green
